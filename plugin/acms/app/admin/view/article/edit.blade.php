            ...
            <div class="main-container">
                <div class="layui-form-item">
                    ...
                    <div class="layui-input-block">
                        <input type="text" name="title" value="{{ $article->title }}" lay-verify="required"
                ...
                <div class="layui-form-item">
                    ...
                    <div class="layui-input-block">
                        <textarea name="summary" placeholder="请输入文章摘要" class="layui-textarea">{{ $article->summary }}</textarea>
                    </div>
                </div>
                <div class="layui-form-item">
                    <label class="layui-form-label">缩略图</label>
                    <div class="layui-input-block">
                        <input type="text" name="thumb" value="{{ $article->thumb }}" placeholder="请输入缩略图URL"
                ...
                <div class="layui-form-item">
                    ...
                    <div class="layui-input-block">
                        <select name="tags" xm-select="tags" xm-select-search="">
                            @foreach ($tags as $tag)
                                <option value="{{ $tag->id }}" @if (in_array($tag->id, $articleTags)) selected @endif>
                                    {{ $tag->name }}</option>
                            @endforeach
                        </select>
                    </div>
                ...
                <div class="layui-form-item">
                    ...
                    <div class="layui-input-block">
                        <input type="text" name="seo_title" value="{{ $article->seo_title }}" placeholder="请输入SEO标题"
                            autocomplete="off" class="layui-input">
                    </div>
                </div>
                <div class="layui-form-item">
                    ...
                    <div class="layui-input-block">
                        <input type="text" name="seo_keywords" value="{{ $article->seo_keywords }}"
                            placeholder="请输入SEO关键词，用英文逗号分隔" autocomplete="off" class="layui-input">
                    </div>
                </div>
                <div class="layui-form-item">
                    <label class="layui-form-label">SEO描述</label>
                    <div class="layui-input-block">
                        <textarea name="seo_description" placeholder="请输入SEO描述" class="layui-textarea">{{ $article->seo_description }}</textarea>
                    </div>
                </div>
                <div class="layui-form-item">
                    <label class="layui-form-label">排序值</label>
                    <div class="layui-input-block">
                        <input type="number" name="sort" value="{{ $article->sort }}" placeholder="数值越大越靠前"
                            autocomplete="off" class="layui-input">
                    </div>
                </div>
                <div class="layui-form-item">
                    ...
                    <div class="layui-input-block">
                        <input type="radio" name="type" value="1" title="Markdown"
                            {{ $article->type == 1 ? 'checked' : '' }}>
                        <input type="radio" name="type" value="2" title="HTML"
                            {{ $article->type == 2 ? 'checked' : '' }}>
                    </div>
                </div>
                <div class="layui-form-item">
    ...
    <script>
        layui.use(['form', 'element', 'select', 'layer'], function() {
            ...
            form.on('submit(article-update)', function(data) {
                ...
                // 发送请求
                $.ajax({
                    url: '/app/admin/acms/article/update/{{ $article->id }}',
                    data: data.field,
                    dataType: 'json',
                    type: 'post',
